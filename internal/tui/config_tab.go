...
func (m configTabModel) parseConfig(cfg map[string]any) []configField {
	...
	fields = append(fields, configField{"Usage Stats Export", "usage-statistics-export-enabled", "bool", fmt.Sprintf("%v", getBool(cfg, "usage-statistics-export-enabled")), nil})
...
func fieldSection(apiPath string) string {
	...
	case "logging-to-file", "logs-max-total-size-mb", "error-logs-max-files", "usage-statistics-enabled", "usage-statistics-export-enabled", "request-log":
